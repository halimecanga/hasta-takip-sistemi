...
import { cardClass, departmentTagClass, outlineButtonClass, primaryButtonClass } from '../styles/uiClasses'

export default function Staff() {
  const location = useLocation()
  const navigate = useNavigate()
  const { staffItems, updateStaffStatus: updateStaffStatusInContext } = useStaff()
  ...
  const confirmStatusUpdate = () => {
    updateStaffStatusInContext(pendingAction.person.id, pendingAction.nextStatus)
  ...
  const actionsFor = (person) => {
    ...
    const edit = { label: 'Düzenle', icon: <Pencil size={15} />, tone: 'neutral', onSelect: showNoop }
