...
export default function Support() {
  ...
  const submit = async () => {
    try {
      setIsSaving(true)
      const result = await supportApi.create(form)
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 gap-5 max-[900px]:grid-cols-1">
        ...
        <section className={`${paddedCardClass} min-w-0`}><div className={sectionTitleClass}><Send size={20} /><div><h3 className={sectionHeadingClass}>Destek Talebi Oluştur</h3><p className={mutedSmallTextClass}>Ekibimize detaylı bir mesaj iletin</p></div></div><div className={singleColumnFormGridClass}><label className={formLabelClass}>Konu<input className={formInputClass} placeholder="Talebinizin konusu" value={form.subject} onChange={(event) => setForm((current) => ({ ...current, subject: event.target.value }))} /></label><label className={formLabelClass}>Kategori<select className={formInputClass} value={form.category} onChange={(event) => setForm((current) => ({ ...current, category: event.target.value }))}><option value="">Kategori seçin</option><option>Teknik Sorun</option><option>Hesap ve Yetki</option><option>Öneri</option></select></label><label className={formLabelClass}>Mesaj<textarea className={textareaClass} placeholder="Talebinizi detaylı olarak açıklayın..." value={form.message} onChange={(event) => setForm((current) => ({ ...current, message: event.target.value }))} /></label></div><button className={`${primaryButtonClass} mt-4`} disabled={isSaving} type="button" onClick={submit}><Send size={18} />{isSaving ? 'Gönderiliyor...' : 'Talebi Gönder'}</button></section>
